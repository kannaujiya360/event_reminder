  ...
  return (
    <Canvas camera={{ position: [0, 0, 10], fov: 55 }} dpr={window.devicePixelRatio}>
      ...
      <Suspense fallback={null}>
        ...
        {/* Optional floating login image */}
        <mesh position={[0, -1, 0]}>
          <planeGeometry args={[4, 4]} />
          <meshStandardMaterial map={new THREE.TextureLoader().load(loginImage)} />
        </mesh>
      </Suspense>
    </Canvas>
  );
      ...
      <motion.div className="container max-w-6xl flex flex-col md:flex-row items-center gap-12 p-6 md:p-12 rounded-3xl bg-white/5 backdrop-blur-xl shadow-[0_30px_90px_rgba(255,255,255,0.25)]">
        ...
        <motion.div className="flex-1 w-full max-w-md p-8 rounded-3xl bg-white/10 backdrop-blur-3xl shadow-[0_25px_60px_rgba(255,255,255,0.2)] border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full py-3 bg-indigo-600 text-white font-bold rounded-3xl shadow-[0_0_25px_rgba(0,0,0,0.3)] hover:scale-105 hover:shadow-[0_0_35px_rgba(0,0,0,0.5)] transition-all"
            >
              Login
            </button>
          </form>
          ...
          <p className="mt-4 text-center text-white/80">
            ...
            <Link to="/signup" className="text-indigo-400 font-medium hover:underline hover:scale-105 transition-all">
              Sign Up
            </Link>
          </p>
        </motion.div>
